...
class IX_Handle
{
    ...
    Internal *make_internal(BufType page)
    {
        ...
        if (!ret)
            cout << "fail to alloc internal" << endl;
        return ret;
    }
        ...
        while (true)
        {
            ...
            if (curheader->isLeaf)
            {
                ...
                if (header.attrType == INTEGER)
                {
                    ...
                    int data = *((int *)(pdata));
                    int i = lower_bound(attr_start, attr_start + curheader->eleNum, data) - attr_start;
                    memmove(attr_start + i + 1, attr_start + i, (curheader->eleNum - i) * header.attrLen);
                    attr_start[i] = data;
                    BufType page_start = leaf->page, slot_start = leaf->slot;
                    memmove(page_start + i + 1, page_start + i, (curheader->eleNum - i) * sizeof(uint));
                    page_start[i] = rid.page;
                    memmove(slot_start + i + 1, slot_start + i, (curheader->eleNum - i) * sizeof(uint));
                ...
                ++curheader->eleNum;
                release_leaf(leaf);
                while ((curheader->isLeaf && curheader->eleNum == header.capacity + 1) || (!curheader->isLeaf && curheader->eleNum == header.order + 1))
                {
                    ...
                    while (parent->child[n] != id && n <= parent->ph->eleNum)
                        ++n;
                    memmove(parent->child + n + 2, parent->child + n + 1, (parent->ph->eleNum - n) * sizeof(uint));
                    memmove((char *)parent->attr + header.attrLen * (n + 1), (char *)parent->attr + header.attrLen * n, header.attrLen * (parent->ph->eleNum - n));
                    ...
                    if (curheader->isLeaf)
                    {
                        // cout << "leaf split " << id << " and " << splitId << endl;
                        Leaf *news = make_leaf(splitPage);
                        Leaf *origin = make_leaf(page);
                        news->ph->isLeaf = true;
                        news->ph->eleNum = curheader->eleNum - (curheader->eleNum >> 1);
                        ...
                        memcpy((char *)(parent->attr) + n * header.attrLen, (char *)(origin->attr) + (curheader->eleNum - 1) * header.attrLen, header.attrLen);
                        release_internal(parent);
                        memcpy(news->attr, (char *)origin->attr + header.attrLen * (curheader->eleNum), news->ph->eleNum * header.attrLen);
                        memcpy(news->page, origin->page + curheader->eleNum, sizeof(uint) * news->ph->eleNum);
                        memcpy(news->slot, origin->slot + curheader->eleNum, sizeof(uint) * news->ph->eleNum);
                        // cout << "dst " << endl;
                    ...
                    {
                        // cout << "internal split " << id << " and " << splitId << endl;
                        Internal *news = make_internal(splitPage);
                        Internal *origin = make_internal(page);
                        news->ph->isLeaf = false;
                        news->ph->eleNum = curheader->eleNum - (curheader->eleNum >> 1) - 1;
                        news->ph->parent = curheader->parent;
                        curheader->eleNum >>= 1;
                        // cout << "src " << endl;
                        // for (int i = 0; i < curheader->eleNum; ++i) {
                        ...
                        // }
                        // cout << endl;
                        memcpy((char *)(parent->attr) + n * header.attrLen, (char *)(origin->attr) + (curheader->eleNum) * header.attrLen, header.attrLen);
                        release_internal(parent);
                        memcpy(news->attr, (char *)origin->attr + header.attrLen * (curheader->eleNum + 1), news->ph->eleNum * header.attrLen);
                        memcpy(news->child, origin->child + curheader->eleNum + 1, sizeof(uint) * (news->ph->eleNum + 1));
                        // cout << "dst " << endl;
                        // for (int i = 0; i < news->ph->eleNum; ++i) {
                        //     // cout << ((int *)news->attr)[i] << ' ';
                        // }
                        ...
                        for (int i = 0; i <= news->ph->eleNum; ++i)
                        {
                            Page_Header *ph = (Page_Header *)bpm->getPage(fID, news->child[i], index);
                            bpm->markDirty(index);
                            ph->parent = splitId;
                        }
            ...
            {
                // cout << "via internal " << id << " with " << curheader->eleNum << " elements" << endl;
    ...
    bool DeleteEntry(void *pdata, const RID &rid)
    {
        ...
        while (true)
        {
            ...
            if (curheader->isLeaf)
            {
                ...
                Leaf *leaf = make_leaf(page);
                // cout << "via leaf " << id << " with " << curheader->eleNum << " elements" << endl;
                if (header.attrType == INTEGER)
                {
                    ...
                    // cout << endl;
                    int data = *((int *)(pdata));
                    auto i = lower_bound(start, start + curheader->eleNum, data) - start;
                    if (i < curheader->eleNum && start[i] == data && leaf->page[i] == rid.page && leaf->slot[i] == rid.slot)
                    {
                        // cout << "found " << start[i] << " at " << i << " in leaf " << id << endl;
                        memmove(start + i, start + i + 1, (curheader->eleNum - i - 1) * sizeof(int));
                        memmove(leaf->page + i, leaf->page + i + 1, (curheader->eleNum - i - 1) * sizeof(uint));
                        memmove(leaf->slot + i, leaf->slot + i + 1, (curheader->eleNum - i - 1) * sizeof(uint));
                        bpm->markDirty(index);
                        ret = true;
                    }
                }
                else if (header.attrType == FLOAT)
                {
                    ...
                    if (i < curheader->eleNum && start[i] == data && leaf->page[i] == rid.page && leaf->slot[i] == rid.slot)
                    {
                        memmove(start + i, start + i + 1, (curheader->eleNum - i - 1) * sizeof(float));
                        memmove(leaf->page + i, leaf->page + i + 1, (curheader->eleNum - i - 1) * sizeof(uint));
                        memmove(leaf->slot + i, leaf->slot + i + 1, (curheader->eleNum - i - 1) * sizeof(uint));
                        bpm->markDirty(index);
                        ret = true;
                ...
                if (curheader->eleNum == 0)
                {
                    ...
                    // curheader = (Page_Header *)bpm->getPage(fID, curheader->parent, index);
                }
                while (curheader->eleNum == 0)
                {
                    ...
                    if (curheader->isLeaf)
                    {
                        uint attr_to_delete = n == parent->ph->eleNum ? parent->ph->eleNum - 1 : n;
                        memmove(parent->child + n, parent->child + n + 1, (parent->ph->eleNum - n) * sizeof(uint));
                        memmove((char *)parent->attr + attr_to_delete * header.attrLen, (char *)parent->attr + (attr_to_delete + 1) * header.attrLen, (parent->ph->eleNum - attr_to_delete - 1) * header.attrLen);
                        --parent->ph->eleNum;
                    }
                    else
                    {
