...
int main()
{
    ...
    cout << "finished insert in " << (double)(c2 - c1) / CLOCKS_PER_SEC << "s" << endl;
    clock_t c3 = clock();
    for (int i: vec) {
        RID ret = {-1, -1};
        if (ih.SearchEntry((void *)(&i), ret))
        {
            ...
            // cout << "found " << i << ' ' << ret.page << ' ' << ret.slot << endl;
        }
        else
        {
    ...
    cout << "finished search in " << (double)(c4 - c3) / CLOCKS_PER_SEC << "s" << endl;
    clock_t c5 = clock();
    for (int i: vec)
    {
        RID ret = {i, i};
        ...
        // {
        //     cout << "delete " << i << ' ' << ret.page << ' ' << ret.slot << endl;
        // }
        // else
        // {
