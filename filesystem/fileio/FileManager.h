...
class FileManager {
   private:
    ...
    int _openFile(const char* name, int fileID) {
        ...
        // cout << "real fid " << f << " fake fid " << fileID << endl;
        if (f == -1) {
            return -1;
        }
        fd[fileID] = f;
   ...
   public:
    ...
    int writePage(int fileID, int pageID, BufType buf, int off) {
        // cout << to_string(fileID) + "." + to_string(pageID) << endl;
        int f = fd[fileID];
        // cout << "write real FID " << f << endl;
        off_t offset = pageID;
        ...
        // FILE* fp =
        //     fopen((to_string(fileID) + "." + to_string(pageID)).c_str(),
        //     "a+");
        // if (fp == NULL) {
        //     cout << "fail" << endl;
        //     return -1;
        ...
        // int tmp =
        //     open((to_string(fileID) + "." + to_string(pageID)).c_str(),
        //     O_RDWR);
        // write(tmp, (void*)b, PAGE_SIZE);
